                               ...
                               int B, int N, int NH, int d)
{
    ...
    int inp_idx = (b * N * stride) + (n * stride) + (0 * NH * d) + (nh_ * d) + d_;

    q[idx] = __ldcs(&inp[inp_idx]);
    //k[idx] = __ldcs(&inp[inp_idx + NH * d]);
    //v[idx] = __ldcs(&inp[inp_idx + ((NH+NH_K) * d)]);
}
                       ...
                       int B, int T, int C, int NH, cudaStream_t stream)
{
    ...
    permute_kernel<<<num_blocks, block_size, 0, stream>>>(q, k, v, inp, B, T, NH, HS);
        ...
        cudaCheck(cudaMemcpy(cpu, q, 33000 * sizeof(floatX), cudaMemcpyDeviceToHost));
        for (int i = 0; i < 9001; i++) {
            printf("q_post_permute_and_rope[%d] = %.8f\n", i, (float) cpu[i]);
        }
        for (int i = 32000; i < 0; i++) {
            printf("k_post_permute_and_rope[%d] = %.8f\n", i, (float) cpu[i]);
        }
        for(int i = B * T * C - 32; i < B * T * C; i++) {
            //printf("y_post_proj[%d] = %.8f\n", i, (float) cpu[i]);
        }
        exit(0);
